...
if(method==="POST"&&path==="/api/rides"){const b=await body(req);if(!b.pickup||!b.destination)return send(res,400,{message:"Pickup and destination are required"});const r={id:randomUUID(),riderId:"demo",driverId:null,status:"requested",pickup:b.pickup,destination:b.destination,rideType:b.rideType||"standard",passengers:Number(b.passengers)||1,fare:fare(b),paymentMethod:"cash_app",paymentStatus:"unpaid",createdAt:new Date().toISOString()};rides.unshift(r);return send(res,201,r)}
if(method==="POST"&&path.match(/^\/api\/rides\/[^/]+\/payment-confirm$/)){const id=path.split("/")[3];const r=rides.find(x=>x.id===id);if(!r)return send(res,404,{message:"Ride not found"});r.paymentStatus="reported_paid";r.paymentConfirmedAt=new Date().toISOString();return send(res,200,r)}if(method==="GET"&&path==="/api/driver/profile")return send(res,200,drivers[0]);
