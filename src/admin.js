...
export async function renderAdmin(){
 ...
 try{
  ...
  stats.innerHTML=[["Active rides",s.activeRides],["Online drivers",s.driversOnline],["Rides today",s.ridesToday],["Revenue",money(s.revenueToday)]].map(x=>'<div class="stat"><b>'+esc(x[1]??0)+"</b>"+x[0]+"</div>").join("");
  adminRides.innerHTML=table(["Status","Rider","Driver","Fare"],(r.rides||r||[]).map(x=>[esc(x.status),esc(x.riderName||"—"),esc(x.driverName||"—"),money(x.fare)]));
  drivers.innerHTML=table(["Driver","Status","Rating"],(d.drivers||d||[]).map(x=>[esc(x.name),x.online?"Online":"Offline",esc(x.rating??"—")]));
